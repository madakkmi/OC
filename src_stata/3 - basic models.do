/****************************************************************************************************************************************
Basic adjusted models for overall OC and for 4 subtypes

Notes:
1. Check and create outcome variables for subtypes if they do not exist
2. Define variables for adjustment, binary, categorical/ordinal, and continuous features
3. Generate descriptive statistics and run logistic regression models for each outcome variable
*****************************************************************************************************************************************/

log using "OC\reports\basic_models.smcl", replace
...
/* Outcome Variables */
local outcome_variables ovarian_cancer subtype_serous_oc subtype_endometrioid_oc subtype_clearcell_oc subtype_mucinous_oc

/* Biomarkers features, to be adjusted with fasting time */
...
/* Biomarkers with aliquotes to be adjusted additionally*/
local biomarkers_with_aliquotes x30620_alt x30650_ast x30690_cl x30710_crp x30720_cycc x30740_glc x30760_hdl x30770_igf1 x30860_tp x30870_tg



local fasttime  _x74_fasttime
...
foreach outcome_var in `outcome_variables' {
    set more off
    tempname results
    postfile `results' str20 model str20 exposure str20 var_type n n_cancer beta se lci uci double p using "OC\reports\basic_binary_and_continuous_for_`outcome_var'.dta", replace

    /* Binary Features */
    foreach col in `binary_features' {
        logit `outcome_var' `col' `adjust_vars'
        count if e(sample)
        local n = r(N)
        ...
        test `col'
        local p = r(p)
        post `results' ("basic") ("`col'") ("binary") (`n') (`n_cancer') (`beta') (`se') (`lci') (`uci') (`p')
    }

    /* Continuous Features */
    foreach col in `continuous_features' {
        
        * check if it is a biomarker
        if strpos("`biomarkers_features'", "`col'") > 0 {
            
            * check if it is a biomarker with aliquot available
            if strpos("`biomarkers_with_aliquotes'", "`col'") > 0 {
                
                * get the name of the aliquot variable, eg, _x30620_aliqout
                local aliquot_var =  substr("`col'",2, strpos("`col'", "_")-2)
                logit `outcome_var' c.`col'  `adjust_vars' c.`fasttime' _x`aliquot_var'_aliquot
                
            }
            else {
                
                logit `outcome_var' c.`col'  `adjust_vars' c.`fasttime'
            }
        }
        else {
            
            logit `outcome_var' c.`col'  `adjust_vars'
        }
        ...
        post `results' ("basic") ("`col'") ("continuous") (`n') (`n_cancer') (`beta') (`se') (`lci') (`uci') (`p')
    }
    ...
    set more off
    tempname results
    postfile `results' str20 model str20 exposure str20 var_type n n_cancer double df double chi2 double p using "OC\reports\basic_categorical_for_`outcome_var'.dta", replace

    /* Categorical/Ordinal Features */
    foreach col in `categorical_features' {
        logit `outcome_var' i.`col' `adjust_vars'
        estimates store m1
        ...
        count if e(sample) & `outcome_var' == 1
        local n_cancer = r(N)
        post `results' ("basic") ("`col'") ("categorical") (`n') (`n_cancer') (`df') (`chi2') (`p')
    }
    ...
    set more off
    tempname results
    postfile `results' str20 model str20 exposure n n_cancer beta se lci uci double p using "OC\reports\basic_categorical_levels_for_`outcome_var'.dta", replace
    ...
    foreach col in `categorical_features' {
        ...
        foreach lev of local levels {
            ...
            if `beta' == 0 & `se' == 0 {
                ...
                local n_cancer = r(N)
                post `results' ("basic") ("`col'=`lev'") (`n') (`n_cancer') (0) (0) (0) (0) (.)
            } else {
                count if `col' == `lev' & e(sample)
                local n = r(N)
                count if `outcome_var' == 1 & `col' == `lev' & e(sample)
                ...
                test `lev'.`col'
                local p = r(p)
                post `results' ("basic") ("`col'=`lev'") (`n') (`n_cancer') (`beta') (`se') (`lci') (`uci') (`p')
            }
        }
    }
