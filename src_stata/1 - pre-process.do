...
rename x20004__1362 x20004__1362_tl
label variable x20004__1362_tl "Operation - sterilisation"
label values x20004__1362_tl yes_no_lab
...
gen x20011_ageopcat =  x20011_ageop
recode x20011_ageopcat (min/24.999999 = 0) (25/34.999999 = 1) (35/44.999999 = 2) (45/max = 3)
label variable x20011_ageopcat "Age when operation took place (caegorical)"
label define x20011_ageopcat_lab  0 "<25" 1 "25-34.9" 2 "35-44.99" 3 "45+"
label values x20011_ageopcat x20011_ageopcat_lab
...
summ x20015_sithgt
replace x20015_sithgt = (x20015_sithgt- r(mean))/r(sd)
...
summ x21002_wt
replace x21002_wt = (x21002_wt- r(mean))/r(sd)
...
replace x2804_ageocplast = x2804_ageocplast / 5
...
summ x50_standhgt
replace x50_standhgt = (x50_standhgt- r(mean))/r(sd)
...
rename _x74 _x74_fasttime
label variable _x74_fasttime "Fasting time"
...
drop x981
merge 1:1 userID using "OC\data\processed\ukb_OC_walking_for_pleasure_raw_data.dta"
drop if _merge != 3
label variable  x981_0_0 "Duration walking for pleasure (modified)"
recode x981_0_0 -3=. -1=. 0=1 3=2 4=2 5=3 6=3 7=4
label define x981_0_0_lab 1 "Non-walkers/Less than 15 minutes" 2 "Between 15 and 1.5 hours" 3 "Between 1.5 hours and 3 hours" 4 "Over 3 hours"
label values x981_0_0 x981_0_0_lab
rename x981_0_0 x981_plwalkdur


* create other outcomes (subtypes and time lag analysis outcomes)

capture confirm variable subtype_serous_oc
if !_rc {
    display "outcome variables for subtypes exists, no need to create"
}
else {
...
replace subtype_endometrioid_oc = . if userID == xxxxxxx  | userID == yyyyyy  | userID == zzzzzz
* only the middle one is available


capture confirm variable ovarian_cancer_aft2yr
if !_rc {
    display "Outcome variable ovarian cancer after 2 years exists, so no need to create it"
}
else {

    gen baseline_date = date(_x53_dtatcentre, "YMD")
    gen oc_diagnos_date = date(date_ocall_first_forgenetic, "YMD")
    gen oc_diagnos_gap = (oc_diagnos_date - baseline_date)/365.25

    gen ovarian_cancer_aft2yr = ovarian_cancer if ovarian_cancer == 0
    replace ovarian_cancer_aft2yr = 1 if  ovarian_cancer == 1 & oc_diagnos_gap > 2

    gen     subtype_serous_oc_aft2yr = 0 if ovarian_cancer == 0
    replace subtype_serous_oc_aft2yr = 1 if subtype_serous_oc == 1 & oc_diagnos_gap > 2

    gen     subtype_endometrioid_oc_aft2yr = 0 if ovarian_cancer == 0
    replace subtype_endometrioid_oc_aft2yr = 1 if subtype_endometrioid_oc == 1 & oc_diagnos_gap > 2

    gen     subtype_clearcell_oc_aft2yr = 0 if ovarian_cancer == 0
    replace subtype_clearcell_oc_aft2yr = 1 if subtype_clearcell_oc == 1 & oc_diagnos_gap > 2 

    gen     subtype_mucinous_oc_aft2yr = 0 if ovarian_cancer == 0
    replace subtype_mucinous_oc_aft2yr = 1 if subtype_mucinous_oc == 1 &  oc_diagnos_gap > 2 
